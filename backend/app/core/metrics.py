...
def compare_search_methods(scan_res: TableScanResult, index_res: dict[str, Any]) -> ComparisonMetrics:
    ...
    """
    index_pages = index_res.get("pages_read", 1)
    index_time = index_res.get("elapsed_time", 0.000001) 
    
    pages_saved = scan_res.pages_read - index_pages
    time_diff = scan_res.elapsed_time - index_time
    
    speedup = scan_res.elapsed_time / index_time if index_time > 0 else 1.0
    
    return ComparisonMetrics(
        key=scan_res.key,
        table_scan=scan_res,
        index_search=index_res,
        pages_saved=max(0, pages_saved),
